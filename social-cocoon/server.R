...
shinyServer(function(input, output, session) {
  new.tweets.df <- reactiveFileReader(1000, session, filePath = "tweets.json", readFunc = parseTweets)
  old.tweets.df <- reactiveFileReader(1000, session, filePath = "tweets.rds", readFunc = readRDS)
  
  data <- reactive({
    tweets.df            <- new.tweets.df()
